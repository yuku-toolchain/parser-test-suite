...
TemporalHelpers.assertPlainTime(plainTime.add({
  hours: 16
...
TemporalHelpers.assertPlainTime(Temporal.PlainTime.from("07:23:30.123456789").add({
  hours: -16
}), 15, 23, 30, 123, 456, 789, "add -16 hours across midnight boundary");
TemporalHelpers.assertPlainTime(Temporal.PlainTime.from("16:08:30.123456789").add({
  minutes: -45
}), 15, 23, 30, 123, 456, 789, "add -45 minutes");
TemporalHelpers.assertPlainTime(Temporal.PlainTime.from("15:36:50.123456789").add({
  seconds: -800
}), 15, 23, 30, 123, 456, 789, "add -800 seconds");
TemporalHelpers.assertPlainTime(Temporal.PlainTime.from("15:23:30.923456789").add({
  milliseconds: -800
}), 15, 23, 30, 123, 456, 789, "add -800 milliseconds");
TemporalHelpers.assertPlainTime(Temporal.PlainTime.from("15:23:30.124256789").add({
  microseconds: -800
}), 15, 23, 30, 123, 456, 789, "add -800 microseconds");
TemporalHelpers.assertPlainTime(Temporal.PlainTime.from("15:23:30.123457089").add({
  nanoseconds: -300
