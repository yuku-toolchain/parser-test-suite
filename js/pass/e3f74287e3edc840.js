...
for (const timeZone of ["Etc/GMT+0", "Europe/Amsterdam", "America/Vancouver"]) {
  ...
  const resultTimeUnit = instance.since(limit, {
    largestUnit: "seconds"
  });
  TemporalHelpers.assertDurationsEqual(limit.since(instance, {
    largestUnit: "seconds"
  }), resultTimeUnit.negated(), `Arithmetic since limit with time largestUnit is self-consistent (${timeZone})`);
}
