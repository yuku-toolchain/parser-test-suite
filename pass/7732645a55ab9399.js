...
}
{
  ...
  const s2 = new MySetLike();
  const result = s1.isSupersetOf(s2);
  const expectedOrder = ["getting size", "ToNumber(size)", "getting has", "getting keys", "calling keys", "getting next", "calling next", "getting done", "getting value", "calling next", "getting done", "getting value"];
}
{
  observedOrder = [];
